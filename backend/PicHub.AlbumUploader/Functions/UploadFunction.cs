...
public class UploadFunction(IAlbumRepository repo, QuotaService quota, FileValidationService validator, BlobServiceClient blobService)
{
    private readonly IAlbumRepository _repo = repo;
    private readonly QuotaService _quota = quota;
    ...
    public async Task<HttpResponseData> UploadMedia([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "albums/{publicToken}")] HttpRequestData req, string publicToken)
    {
        ...
        var container = _blobService.GetBlobContainerClient("pichub-local");
        ...
        while (section != null)
        {
            ...
            if (hasContentDispositionHeader && contentDisposition.DispositionType.Equals("form-data") && !string.IsNullOrEmpty(contentDisposition.FileName.Value))
            {
                ...
                if (!string.IsNullOrEmpty(sectionContentType) && !FileValidationService.IsContentTypeAllowed(sectionContentType))
                {
